        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1 }} className="flex flex-wrap gap-2 mb-5">
          {projects.map(p => (
            <button key={p.id} onClick={() => { setActiveProject(p); setActiveIdx({ type: "builtin", idx: 0 }) }}
              className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-sm font-semibold border transition-all ${activeProject.id === p.id ? "text-[#0d1117] border-transparent" : "glass border-[#30363d] text-slate-400 hover:text-white hover:border-cyan-500/40"}`}
